...
def refresh_access_token():
    r = requests.post("https://kauth.kakao.com/oauth/token",
        data={"grant_type":"refresh_token","client_id":REST,"refresh_token":RT}, timeout=10)
    r.raise_for_status()
...
def format_message(df: pd.DataFrame) -> str:
    ...
    for i, r in enumerate(df.head(5).to_dict("records"), 1):
        name=r.get("name",""); price=r.get("price",""); stop=r.get("stop",""); target=r.get("target","")
...
def main():
    ...
    csv_text = requests.get(CSV, timeout=10).text
